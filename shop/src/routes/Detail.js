...
const Detail = ({ shoes }) => {
	...
	return (
		<div className='container'>
			...
			<TabData tab={tab} shoes={shoes} />
		</div>
	);
};
//if문은 위의 HTML에 쓸수가 없다.
//일반 if 조건문을 쓰려면 컴포넌트로 만들어서 위에 넣는다.
function TabData({ tab, shoes }) {
	//컴포넌트는 return을 안넣으면 제 기능을 못한다.
	...
	// 	return <div>내용2</div>;
	// }
	//array에 넣어서 state에 따라 맞는 값을 꺼내는 식으로 짜면 if문 없이 사용할 수 있다.
	// [<div>내용0</div>, <div>내용0</div>, <div>내용0</div>][0];

	//tab state가 변하면 end를 부착할거다.
	...
	//state변경하는게 근처에 있다면, state 변경이다된후합쳐서 한번만 렌더링 한다.
	//그래서 setTimeout을 줫음
